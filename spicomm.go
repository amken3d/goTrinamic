...
type SPIComm struct {
	spi    SPI
	CsPins map[uint8]Pin // Map to store CS pin for each Driver by its address
}

// NewSPIComm creates a new SPIComm instance.
func NewSPIComm(spi SPI, csPins map[uint8]Pin) *SPIComm {
	return &SPIComm{
		spi:    spi,
		CsPins: csPins,
	}
...
func spiTransfer40(spi SPI, register uint8, txData uint32) (uint32, error) {
